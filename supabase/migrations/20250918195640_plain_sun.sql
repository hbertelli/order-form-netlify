/*
  # Create Order Number System

  1. New Tables
    - Creates sequence for auto-incrementing order numbers starting at 1000
    - Adds order_number column to orders_submitted table
    - Adds estimated_order_number to order_sessions table
  
  2. Functions
    - get_next_order_number(): Returns the next order number for preview
  
  3. Triggers
    - Auto-assigns order numbers when orders are submitted
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE orders_submitted ADD COLUMN order_number INTEGER UNIQUE DEFAULT nextval('order_number_seq');
  END IF;
END $$;

-- Add estimated_order_number to order_sessions table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
